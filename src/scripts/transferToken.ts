...
class TokenTransferer {
  ...
  private async sendTransaction(
    ...
    const rawTransaction = {
      ...
      nonce:
        '0x' +
        new BigNumber((await this.getNonce(senderAddress)) + 4).toString(16),
  ...
  async sendTokens(to: string, value: string) {
    const intValue = ethers.utils.bigNumberify(
      this.integerize(value).toString(),
    );
...
(async () => {
  ...
  try {
    const arcadeResults = await arcadeSender.sendTokens(
      toAddress,
      amountToSend,
    );
