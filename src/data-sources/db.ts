...
abstract class Db extends DataSource {
  ...
  async create(object: AnyObj) {
    try {
      ...
      logger.warn(`data-sources.db.${this.model.modelName}.create.catch:${error}`)
      throw error;
    }
  }
  ...
  async findById(id: string) {
    try {
      logger.debug(`data-sources.db.${this.model.modelName}.findById:${id}--start`)
      const res = await this.model.findById(id);
      logger.debug(`data-sources.db.${this.model.modelName}.findById:${id}--done`)
      return res ? res : {};
    } catch (error) {
      logger.warn(`data-sources.db.${this.model.modelName}.findById.catch:${error}`)
      throw error;
    }
  }
  ...
  async findOne(query: FilterQuery<AnyObj>) {
    try {
      ...
      logger.warn(`data-sources.db.${this.model.modelName}.findOne.catch:${error}`)
      throw error;
    }
  }
  ...
  async find(query: FilterQuery<AnyObj>) {
    try {
      ...
      logger.warn(`data-sources.db.${this.model.modelName}.find.catch:${error}`)
      throw error;
    }
  }
  ...
  async updateById(id: string, updates: AnyObj) {
    try {
      logger.debug(`data-sources.db.${this.model.modelName}.updateById:${id}--start`)
      const res = await this.model.findByIdAndUpdate(id, updates);
      logger.debug(`data-sources.db.${this.model.modelName}.updateById:${id}--done`)
      return res ? res : {};
    } catch (error) {
      logger.warn(`data-sources.db.${this.model.modelName}.updateById.catch:${error}`)
      throw error;
    }
  }
  ...
  async updateOne(query: FilterQuery<AnyObj>, updates: AnyObj) {
    try {
      ...
      logger.warn(`data-sources.db.${this.model.modelName}.updateOne.catch:${error}`)
      throw error;
    }
  }
  ...
  async updateMany(query: FilterQuery<AnyObj>, updates: AnyObj) {
    try {
      logger.debug(`data-sources.db.${this.model.modelName}.updateMany:--start`)
      const res = await this.model.updateMany(query, updates);
      logger.debug(`data-sources.db.${this.model.modelName}.updateMany:--done`)
      return Array.isArray(res) ? res.filter(r => r) : [];
    } catch (error) {
      logger.warn(`data-sources.db.${this.model.modelName}.updateManu.catch:${error}`)
      throw error;
    }
  }
  ...
  async deleteById(id: string) {
    try {
      logger.debug(`data-sources.db.${this.model.modelName}.deleteById:${id}--start `)
      const res = await this.model.findByIdAndDelete(id);
      logger.debug(`data-sources.db.${this.model.modelName}.deleteById:${id}--done `)
      return res ? res : {};
    } catch (error) {
      logger.warn(`data-sources.db.${this.model.modelName}.deleteById.catch:${error}`)
      throw error;
    }
  }
  ...
  async aggregate(pipeline: AnyObj[]) {
    try {
      ...
      logger.warn(`data-sources.db.${this.model.modelName}.aggregate.catch:${error}`)
      throw error;
    }
  }
}
